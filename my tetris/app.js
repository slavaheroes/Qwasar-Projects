...
let lines = 0; // Tracks current # of lines
...
let nextTetromino = [[0, 0], [1, 0], [0, 1], [1, 1]];
...
let tetrominoColors = ["cyan", "yellow", "purple", "green", "red", "blue", "orange"];

// 3. Holds current Tetromino color
let curTetrominoColor;
let nextTetrominoColor = "purple";

// 4. Create gameboard array so we know where other squares are
let gameBoardArray = [...Array(20)].map(e => Array(10).fill(0));

// 6. Array for storing stopped shapes
// It will hold colors when a shape stops and is added
let stoppedShapeArray = [...Array(20)].map(e => Array(10).fill(0));
...
function SetupCanvas() {
    ...
    ctx.strokeRect(8, 8, 236, 462);


    //Setting up canvas for next tetramino
    canvas1 = document.getElementById('nextTetramino');
...
//Drawing Next Tetramino in different canvas
function DrawNextTetramino(){
    ...
    for (let i = 0; i < nextTetromino.length; i++) {
        ...
        // Move the Tetromino x & y values to the tetromino
        // shows in the middle of the gameboard
        let x = 25*nextTetromino[i][0] - 1;
        let y = 25*nextTetromino[i][1] - 1;

        // 1. Draw a square at the x & y coordinates that the lookup
        // table provides
        ctx1.fillStyle = nextTetrominoColor;
        ctx1.fillRect(x, y, 21, 21);
    }
}

//Deleting Next Tetramino in different canvas
function DeleteNextTetramino(){
    for (let i = 0; i < nextTetromino.length; i++) {
        let x = 25*curTetromino[i][0] - 1;
        let y = 25*curTetromino[i][1] - 1;
